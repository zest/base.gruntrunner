var path = require('path'),
    merge = require('merge'),
    modulePath = './base.gruntrunner/node_modules';
module.exports = function (grunt) {
    ...
    grunt.loadNpmTasks(path.join(modulePath, 'grunt-contrib-clean'));
    // ...for watching for file changes
    grunt.loadNpmTasks(path.join(modulePath, 'grunt-contrib-watch'));
    // ...for code quality
    grunt.loadNpmTasks(path.join(modulePath, 'grunt-contrib-jshint'));
    // ...for generating jsdoc documentation
    grunt.loadNpmTasks(path.join(modulePath, 'grunt-jsdoc'));
    // ...for running node mocha tests and code coverage reports
    grunt.loadNpmTasks(path.join(modulePath, 'grunt-mocha-cov'));
    // ...for generating github website
    grunt.loadNpmTasks(path.join(modulePath, 'grunt-gh-pages'));
    ...
    grunt.initConfig({
        ...
        jsdoc: {
            lib: {
                ...
                options: {
                    configure: path.join('node_modules', modulePath, '../lib/jsdoc.json'),
                    template: path.join('node_modules', modulePath, './ink-docstrap/template')
                },
                jsdoc: path.join('node_modules', modulePath, './grunt-jsdoc/node_modules/jsdoc/jsdoc')
            }
        },
        'gh-pages': {
            options: {
                ...
                message: 'auto-commit <%= pkg.name %> on ' + new Date()
            },
            src: [
                '**'
            ]
